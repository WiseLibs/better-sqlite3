...
var closeDatabase = CPPDatabase.prototype.close;
CPPDatabase.prototype.close = function close(options) {
	if (options && options.optimize && this.open) {
		var mask = typeof options.optimize === 'number' ? options.optimize.toString(16) : 'fffe';
		this.exec('PRAGMA optimize(0x' + mask + ')');
	}
