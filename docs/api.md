...
It's important to understand that because the database can be modified between transfer operations, the returned object only provides an *estimate* of the backup progress. Also, if the source database is busy being used by another process, it's possible for this method to return without actually transferring any pages, regardless of the desired `pageCount` (an error will *not* be thrown).
