...
It's important to understand that because the database can be modified between transfer operations, the returned object only provides an *estimate* of the backup progress. Also, if the source database is being used by another process simultaneously, it's possible for this method to return "successfully" without actually transferring any pages (i.e., because the database was busy), regardless of the desired `pageCount`.
