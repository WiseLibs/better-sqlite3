...
# If you see a "Bad substitution" error on the following line, you've ignored
# the shebang and aren't using bash:
MAJOR=${VERSION:0:1}
...
printf '%s' "$DEFINES" | sed -e "/^\s*$/d" -e "s/\(.*\)/    '\1',/" >>"$GYP"
printf "  ],\n}\n" >>"$GYP"
...
printf '%s' "$DEFINES" | sed -e "/^\s*$/d" >>"$DOCS"
printf "\`\`\`\n" >>"$DOCS"
